...
class T1PIController:
    ...
    def __init__(self, config: ControllerConfig, name: str = None):
        ...
        self.input_names = (
            ["T1SP"] + [f"T1meas{i + 1}" for i in range(nl)] + ["Tin"]
        )
...
class T2GMCController:
    ...
    def _compute(self, x, u):
        ...
        T2SP, T2meas, Iradmeas, epsilonest, Tin, Fmeasfilt = (
            self._unpack_input(u)
        )
...
class FMBCController:
    ...
    def __init__(self, config: ControllerConfig, name: str = None):
        ...
        self.nu = 3 * nl + 1
        self.ny = nl
        self.state_names = [f"valvexm{i + 1}" for i in range(nl)] + [
    ...
    def _compute(self, x, u):
        ...
        Fm = (
            ...
            * cas.sqrt(
                ...
                / (
                    cfg.G
                    + (flowaest + 9.0 * cfg.Flowb) * (cfg.Cv * fofxm) ** 2
                )
            )
        )
        ...
        fofxdesired = _where(
            denom > 0.0, cas.sqrt(cfg.G / cfg.Cv**2 / denom), 1.0
        )
...
class PumpPIController:
    ...
    def _compute(self, x, u):
        ...
        pumpintegral_new = (
            pumpintegral + cfg.dt_ctrl * pumppropor / cfg.PumpPITau
        )
        spumpstarg = _clip(
